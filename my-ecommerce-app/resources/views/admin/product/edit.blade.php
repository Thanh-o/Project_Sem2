    ...
    <div class="wrapper">
      ...
      <div class="main-panel">
          ...
          <div class="page-inner">
                                ...
                                <form action="{{ route('admin.product.update', $product->product_id) }}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <img src="{{ asset('storage/' . $image->file_path) }}" alt="Product Image" style="width: 100px; height: 100px; border-radius: 5px;">
                                            @else
                                            <video src="{{ asset('storage/' . $image->file_path) }}" controls style="width: 100px; width: 100px; height: 100px; border-radius: 5px;"></video>
                                            @endif
                                            <input type="checkbox" name="deleted_images[]" value="{{ $image->image_id }}"> Delete
                                        </div>
                                        @endforeach
                                        <div id="mediaUploadContainer"></div>
                                        <button type="button" class="btn btn-success btn-sm mt-2" onclick="addMediaField()">Add Image</button>
                                    </div>
            ...
            <script>
                ...
                function addMediaField() {
                    mediaIndex++;
                    let container = document.getElementById('mediaUploadContainer');
                    let inputField = document.createElement('input');
                    inputField.type = 'file';
                    inputField.name = 'file_path[]';
        ...
        if (confirm('Are you sure you want to delete this image?')) {
            $.ajax({
                url: '{{ route('admin.product.update', $product->product_id) }}',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    _method: 'PUT',
                    deleted_images: [imageId]
                },
                success: function(response) {
                    // Assuming you want to remove the deleted image from UI
